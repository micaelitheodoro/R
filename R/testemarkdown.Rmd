...
dados %>%
  ...
  ggplot()+
    geom_col(aes(x = type, y = media_altura))
...
dados %>%
  ...
  tidyr::pivot_longer(cols = c("media_altura", "media_peso"), names_to = "tipo", values_to = "media") %>% 
  ggplot()+
...
fator <- max(df$media_peso)/max(df$media_altura)
fator

df$media_altura <- df$media_altura*fator
...
df_media <- df %>% 
  select(type, media_altura, media_peso) %>% 
  tidyr::pivot_longer(cols = c("media_altura", "media_peso"), names_to = "tipo", values_to = "media") %>%
  mutate(
    tipo = ifelse(tipo == "media_altura", "Altura", "Peso")
  )
...
df_long <- df %>%
  pivot_longer(cols = c(media_altura, media_peso), names_to = "tipo", values_to = "media")

ggplot(df_long, aes(x = type, y = media, color = tipo, fill = tipo)) +
  ...
  scale_color_manual(values = c("#6E0DAA", "#AE57A4")) +  
  scale_fill_manual(values = c("#6E0DAA", "#AE57A4")) +   
  theme_bw() +
  theme(
